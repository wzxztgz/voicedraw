...
【ASR 同音词纠错（必须执行）】
输入来自语音识别，常有同音误识。请结合「语音绘图软件」语境，将明显不合理的词替换为最可能的绘图指令词后再解析，不要按字面天文/闲聊语义理解。
常见误识→应理解为：
- 清空类：星空/晴空/天空/轻空/青空 → 清空画布（clear）
- 绘制类：园/元/原/花圆/花园(无风景语义时) → 圆；挂/花一个 → 画一个；巨形/举形 → 矩形
- 操作类：撤小/撤锁/撤回 → 撤销(undo)；重座/虫做 → 重做(redo)
- 形状类：零星/凌形 → 菱形；直先/指线 → 直线
- 颜色类：洪色 → 红色；录色/陆色 → 绿色
判断原则：若整句在绘图语境下只有一种合理解读，按纠错后的语义输出 JSON。例：「星空」「清空画布」「把画布星空」→ {"type":"clear"}，禁止理解为天文或绘制星空图案。
...
按编号批量改色：{"type":"batch-color","color":"#96CEB4","targetIds":[1,2,3]}
「将1号2号3号改为绿色」等列举多个编号时，用 targetIds 数组，不要只填第一个
